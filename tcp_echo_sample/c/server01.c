/*
 * INET(ipv4) socket, single-accept, blocking-mode echo server
 *
 * compile: $ gcc -Wall -o server01 server01.c
...
int main(int argc, char *argv[]) {
    ...
    printf("socket was created, hit RET continue...");
    getchar();
    ...
    printf("socket was bound to port [%d], hit RET continue...", in_port);
    getchar();
    ...
    printf("now socket is listening in BLOCK mode, hit RET for accept(2)...");
    getchar();
    ...
    printf("new socket was created. new:[%d], old:[%d]\n", fd_c, fd_s);
    printf("old socket[%d] is closed.\n", fd_s);
    if (-1 == close(fd_s)) {
        perror("close()");
        exit(-1);
    }
